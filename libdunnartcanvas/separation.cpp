...
//static void sephandle_handler(QWidget **object_addr, int action);
...
void Separation::initialiser(void)
{
    ...
    // QT hand[0]->set_handler(sephandle_handler);
...
#if 0
void Separation::draw(SDL_Surface *surface, const int x, const int y,
        const int t)
{
    if (!surface)
    {
    ...
    if (connectedObjs[0] && connectedObjs[1])
    {
        ...
        int offsetX = x - xpos;
        int offsetY = y - ypos;
        int x1 = connectedObjs[0]->x() + offsetX;
        int x2 = connectedObjs[1]->x() + offsetX;
        int y1 = connectedObjs[0]->y() + offsetY;
        int y2 = connectedObjs[1]->y() + offsetY;
        ...
        clipRect.x = std::min(x1, x2) + 2;
        clipRect.y = std::min(y1, y2) + 2;
        clipRect.w = std::max(x1 + w1, x2 + w2) - clipRect.x - 2;
        clipRect.h = std::max(y1 + h1, y2 + h2) - clipRect.y - 2;
        clipRectPtr = &clipRect;
    }
    
    QColor highlight_col = 0;
    QColor drawtype = DRAW_NORMAL;
    QColor GLOW = QColor(255, 80, 0, 128);
    ...
    switch (t)
    {
        ...
        case SHAPE_STATE_GUIDE_CONNECT:
            if ((highlight_col == 0) || (highlight_col == GLOW))
            {
                highlight_col = QColor(255, 0, 0, 255);
            }
            drawtype |= DRAW_GLOW;
            // Fall-through
        case SHAPE_DRAW_NORMAL:
            if (t == SHAPE_DRAW_HIGHLIGHTED)
            {
                // We are drawing highlighted but need a little bit of
                // extra space at the ends of the separation for the handles.
                drawtype |= DRAW_HANDLES;
            }
            if (cascade_glow)
            {
                // Light highlight the rest of the indicator for queries.
                QColor col = shLightGlow;
                drawSeparation(surface, x, y, NULL, drawtype, col);
            }
            drawSeparation(surface, x, y, clipRectPtr, drawtype, highlight_col);
            break;
        case SHAPE_DRAW_OUTLINE:
            drawtype = DRAW_DASHED;
            drawSeparation(surface, x, y, clipRectPtr, drawtype, highlight_col);
            break;
        default:
            break;
...
void Separation::drawSeparation(SDL_Surface *surface, const int x, const int py,
        SDL_Rect *clipRectPtr, const QColor drawtype,
        const QColor highlight_col)
{
    QColor main_col = QColor(0, 0, 255, alpha);
    ...
    if (drawtype & DRAW_DASHED)
    {
        main_col = QColor(0, 0, 0, 255);
        size = 1;
        skip = 2;
    }
    ...
    if (type == GUIDE_TYPE_VERT)
    {
        ...
        {
            int gp1 = (*r)->guide->x() + 3;
            int gp2 = (*r)->guide2->x() + 3;
            
            int pos1 = x + 3 + std::min(gp1, gp2) - xpos;
            int pos2 = x - 3 + std::max(gp1, gp2) - xpos;

            int ourDrawType = drawtype;
            QColor colour = main_col;
            if ((pos2 - pos1 + 6) <= (int) gap)
            {
                colour = red;
            }
            else
            {
                ourDrawType |= DRAW_DASHED;
            }
            ...
            UTIL_Draw_HLine(surface, pos1, y + 8, pos2, colour,
                    ourDrawType, highlight_col, clipRectPtr, size, skip);
            UTIL_Draw_VLine(surface, pos1, y + 6, y + 10, colour,
                    ourDrawType, highlight_col, clipRectPtr, size, skip);
            UTIL_Draw_VLine(surface, pos2, y + 6, y + 10, colour,
                    ourDrawType, highlight_col, clipRectPtr, size, skip);

            if ((pos2 - pos1 + 6) > (int) gap)
            {
                int half = (int) (gap / 2);
                int halfBetween = (pos2 - pos1) / 2;
            
                UTIL_Draw_VLine(surface, pos1 + halfBetween - half,
                        y + 6, y + 10, colour, drawtype, highlight_col,
                        clipRectPtr, size, skip);
                UTIL_Draw_VLine(surface, pos1 + halfBetween + half,
                        y + 6, y + 10, colour, drawtype, highlight_col,
                        clipRectPtr, size, skip);
                UTIL_Draw_HLine(surface, pos1 + halfBetween - half,
                        y + 8, pos1 + halfBetween + half, colour,
                        drawtype, highlight_col, clipRectPtr, size, skip);

                if (r == rels.begin())
                {
                    handle_x = pos1 + halfBetween - half - x - 3;
                    handles.front()->reposition();
                }
            }
        }
    ...
    else // if (type == GUIDE_TYPE_HORI)
    {
        ...
        {
            int gp1 = (*r)->guide->y() + 3;
            int gp2 = (*r)->guide2->y() + 3;
            
            int pos1 = y + 3 + std::min(gp1, gp2) - ypos;
            int pos2 = y - 3 + std::max(gp1, gp2) - ypos;

            int ourDrawType = drawtype;
            QColor colour = main_col;
            if ((pos2 - pos1) <= (int) gap)
            {
                colour = red;
            }
            else
            ...
            UTIL_Draw_VLine(surface, x + 8, pos1, pos2, colour,
                    ourDrawType, highlight_col, clipRectPtr, size, skip);
            UTIL_Draw_HLine(surface, x + 6, pos1, x + 10, colour,
                    ourDrawType, highlight_col, clipRectPtr, size, skip);
            UTIL_Draw_HLine(surface, x + 6, pos2, x + 10, colour,
                    ourDrawType, highlight_col, clipRectPtr, size, skip);
            ...
            if ((pos2 - pos1) > (int) gap)
            {
                ...
                UTIL_Draw_HLine(surface, x + 6, pos1 + halfBetween - half,
                        x + 10, colour, drawtype, highlight_col,
                        clipRectPtr, size, skip);
                UTIL_Draw_HLine(surface, x + 6, pos1 + halfBetween + half,
                        x + 10, colour, drawtype, highlight_col,
                        clipRectPtr, size, skip);
                UTIL_Draw_VLine(surface, x + 8, pos1 + halfBetween - half,
                        pos1 + halfBetween + half, colour,
                        drawtype, highlight_col, clipRectPtr, size, skip);

                if (r == rels.begin())
                {
                    handle_y = pos1 + halfBetween - half - y - 3;
                    handles.front()->reposition();
                }
            }
        }
