...
#if 0
void Distribution::draw(SDL_Surface *surface, const int xp, const int yp, const int t)
{
    if (!surface)
    {
        return;
    ...
    switch (t)
    {
        ...
        case SHAPE_DRAW_NORMAL:
            if (t == SHAPE_DRAW_HIGHLIGHTED)
            {
                ...
                // extra space at the ends of the distro for the handles.
            ...
            if (cascade_glow)
            {
                ...
                drawDistro(surface, x, y, NULL, drawtype, col);
            }
            drawDistro(surface, x, y, clipRectPtr, drawtype, highlight_col);
            break;
...
void Distro::drawDistro(SDL_Surface *surface, const int x, const int y,
        SDL_Rect *clipRectPtr, const QColor drawtype,
        const QColor highlight_col)
{
    QColor main_col = QColor(0, 0, 255, alpha);
    int halfSep = abs((int) (space / 2));
    bool rev = is_reversed();
    int sections = (int) rels.size();
    ...
    if (type == GUIDE_TYPE_VERT)
    {
        if (sections > 1)
        {
            UTIL_Draw_HLine(surface, x + halfSep + 3, y + 3,
                    x + width - (halfSep + 5), main_col, drawtype,
                    highlight_col, clipRectPtr, size, skip);
        }
        ...
        for (RelsList::iterator r = rels.begin(); r != finish; ++r)
        {
            ...
            int pos1 = x + 7 + std::min(gp1, gp2) - x();
            int pos2 = x - 1 + std::max(gp1, gp2) - x();
            
            if (sections > 1)
            {
                UTIL_Draw_VLine(surface, pos1 + halfSep - 4, y + 3, y + 8,
                        main_col, drawtype, highlight_col, clipRectPtr,
                        size, skip);
            }

            if (drawtype & DRAW_HANDLES)
            {
                // Need extra space for handles.
                ...
                else if ((*r == rels.back()) && rev)
                {
                    pos1 += 3;
                }
            }
            UTIL_Draw_HLine(surface, pos1, y + 8, pos2, main_col,
                    drawtype, highlight_col, clipRectPtr, size, skip);
            UTIL_Draw_VLine(surface, pos1, y + 6, y + 10, main_col,
                    drawtype, highlight_col, clipRectPtr, size, skip);
            UTIL_Draw_VLine(surface, pos2, y + 6, y + 10, main_col,
                    drawtype, highlight_col, clipRectPtr, size, skip);
        }
    }
    else // if (type == GUIDE_TYPE_HORI)
    {
        if (sections > 1)
        {
            UTIL_Draw_VLine(surface, x + 3, y + halfSep + 3,
                    y + height - (halfSep + 5), main_col, drawtype,
                    highlight_col, clipRectPtr, size, skip);
        }
        ...
        for (RelsList::iterator r = rels.begin(); r != finish; ++r)
        {
            ...
            int pos1 = y + 7 + std::min(gp1, gp2) - y();
            int pos2 = y - 1 + std::max(gp1, gp2) - y();
          
            if (sections > 1)
            {
                UTIL_Draw_HLine(surface, x + 3, pos1 + halfSep - 4, x + 8,
                        main_col, drawtype, highlight_col, clipRectPtr,
                        size, skip);
            }
            ...
            UTIL_Draw_VLine(surface, x + 8, pos1, pos2, main_col,
                    drawtype, highlight_col, clipRectPtr, size, skip);
            UTIL_Draw_HLine(surface, x + 6, pos1, x + 10, main_col,
                    drawtype, highlight_col, clipRectPtr, size, skip);
            UTIL_Draw_HLine(surface, x + 6, pos2, x + 10, main_col,
                    drawtype, highlight_col, clipRectPtr, size, skip);
        }
    }
}
