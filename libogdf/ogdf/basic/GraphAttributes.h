 ...
 * last checkin:
 *   $Author: gutwenger $ 
 *   $Date: 2009-07-10 06:58:06 +1000 (Fri, 10 Jul 2009) $ 
 ***************************************************************/
...
protected:
	...
	NodeArray<String>       m_nodeImageUrl;  //!< URL of image for a node
	NodeArray<String>       m_nodeCluster;  //!< URL of image for a node
...
public:
	...
	enum {
        ...
        nodeImageUrl     = 0x40000,  //!< node attribute m_nodeImageUrl
        nodeCluster      = 0x80000  //!< node attribute m_nodeCluster
	};
  ...
  //! Returns a URL for an image associated with the node
  const String &imageUrlNode(node v) const {return m_nodeImageUrl[v];}
  const String &clusterNode(node v) const {return m_nodeCluster[v];}
  //! Returns a URL for an image associated with the node
  String &imageUrlNode(node v) {return m_nodeImageUrl[v];}
  String &clusterNode(node v) {return m_nodeCluster[v];}
