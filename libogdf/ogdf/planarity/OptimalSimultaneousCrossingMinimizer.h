...
//				#endif
	...
	public:
		...
		Master() :
				ABA_MASTER("OptimalSimultaneousCrossingMinimizer::Master",1, true ,ABA_OPTSENSE::Min),
...
*/
	...
	private:
		...
		int getCost(edge e1, edge e2) { // call with e1, e2 of minimizedGraph
			if(useSubgraphs()) {
				unsigned int n = subgraphs[e1]&subgraphs[e2];
				register unsigned int tmp;				    // Counting by MIT & X11: http://infolab.stanford.edu/~manku/bitcount/bitcount.html
...
/*		int m_numStartKuratowskis;
		...
		OptimalSimultaneousCrossingMinimizer::CrossingConfiguration* helpCall(PlanRep &_PG, int cc, const EdgeArray<int>& _cost, const EdgeArray<bool> & _forbid, const EdgeArray<unsigned int> & _subgraphs, int& crossingNumber);
		void generateMinimizedGraphCostForbidAndSubgraphs(const EdgeArray<int>  &_cost, const EdgeArray<bool> &_forbid, const EdgeArray<unsigned int> &_subgraphs);
