 ...
 * last checkin:
 *   $Author: klein $ 
 *   $Date: 2010-01-25 21:15:33 +1100 (Mon, 25 Jan 2010) $ 
 ***************************************************************/
...
#include <ogdf/internal/cluster/MaxCPlanar_ChunkConnection.h>
...
void Master::terminateOptimization() {
	...
	if(m_checkCPlanar2) {
		Logger::slout() << "*** " << (dualBound()!=-infinity() ? "" : "NON ") << "C-PLANAR ***\n";
	} else {
	...
	Logger::ssout() << "\n";
	bool isCp = ((m_checkCPlanar2) ? (dualBound()!=-infinity()) : feasibleFound());
	if (m_checkCPlanar2 && m_deletedOriginalEdges.empty() && !isCp)
		m_deletedOriginalEdges.pushBack(m_G->firstEdge());
