...
void ClusterGraph::moveCluster(cluster c, cluster newParent)
{
	...
	//get old subtree predecessor and successor
	cluster leftc, oldPred, oldSucc;
	if (!newOrder)
	{
		leftc = leftMostCluster(c);
		oldPred = leftc->pPred();
		oldSucc = c->pSucc();
	}
...
/*
	{
		//get new predecessor
		cluster newPred = postOrderPredecessor(c);

		//remember that all pred/succ values can be zero
		
		//we rely on the fact that new and old pred cannot be zero
		//at the same time
		//update new successor and predecessor
		if (newPred)
		{
			c->m_pNext = newPred->pSucc();
			newPred->m_pNext = leftc; //leftmost cluster in subtree
			leftc->m_pPrev = newPred;
		}
		else //leftmost subtree at c
		{
			if (leftc != m_postOrderStart) //leftmost before?
			{
				c->m_pNext = m_postOrderStart;
				//else c->m_pNext = bleibt gleich
				m_postOrderStart->m_pPrev = c;
				m_postOrderStart = leftc;
				leftc->m_pPrev = 0;
			}//if left postorder
			else 
			{
		...
		//update postorderstart if necessary
		if ((oldPred == 0) && (newPred != 0))
			m_postOrderStart = (oldSucc ? oldSucc : leftc);

		//update old predecessor and successor
		//two cases: we hang c at the old successor (leaf or parent) or not
		//can not be old parent because newparent != oldparent
		//this code is not yet correct
		if (oldPred)
		{
			if (newParent == oldSucc)
				oldPred->m_pNext = leftc;
			else
				oldPred->m_pNext = oldSucc;
		}
		if (oldSucc) 
		{
			//if we hang the subtree at the old successor,
			//his predecessor is still c (pushhBack!), not oldpred
			if (newParent != oldSucc)
			//if ( (oldPred != 0) || (leftc != m_postOrderStart))
				oldSucc->m_pPrev = oldPred;
		}
		
		
		OGDF_ASSERT(m_postOrderStart->m_pPrev == 0)
		
	}//else only minimal reordering
*/	
	m_adjAvailable = false;
