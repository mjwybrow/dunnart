...
int OptimalSimultaneousCrossingMinimizer::Subproblem::generateBranchRules(ABA_BUFFER<ABA_BRANCHRULE*> &rules) {
...
bool OptimalSimultaneousCrossingMinimizer::Subproblem::checkSimplicitiesAndPrice() {
...
void OptimalSimultaneousCrossingMinimizer::Subproblem::deactivateNonLiftableConstraints(ABA_BUFFER<ABA_VARIABLE*>* buf) {
	int remd = 0;
	for(int i = nCon(); i-->0;) {
		bool reset = false;
...
Module::ReturnType OptimalSimultaneousCrossingMinimizer::Master::doCall(PlanRep &_PG,
	...
	if(effectiveLogLevel() >= LL_ALARM || effectiveStatisticMode())
		outLevel(OptimalSimultaneousCrossingMinimizer::Master::Silent);
	else
		outLevel(OptimalSimultaneousCrossingMinimizer::Master::Full);
...
void OptimalSimultaneousCrossingMinimizer::Master::doWriteBestSolution() {
	...
	if(writeResult()) {
		...
		GraphAttributes GAR(RR, GraphAttributes::nodeGraphics | GraphAttributes::nodeColor | GraphAttributes::edgeGraphics | GraphAttributes::edgeColor );
		node n;
		edge e;
		...
		if(!useSubgraphs()) {		
				...
				if(!RRR.original(RR.original(n))) {
					if( type=subgraphs[ expandedGraph->original( RRR.original( RR.original(n->firstAdj()->theEdge()).front() )) ] & 
						subgraphs[ expandedGraph->original( RRR.original( RR.original(n->lastAdj()->theEdge()).front() )) ] ) {
			  			GAR.colorNode(n) = "#000000";
						++proper;
						++scr;
						if(type==3) ++scr;
					} else {
						GAR.colorNode(n) = "#AAAAAA";
						++phantom;
					}
					++ccc;
				} else {
					GAR.colorNode(n) = "#FFFF00";
			  	}
			}
			...
			forall_edges(e, RR) {
				unsigned int sub = subgraphs[ expandedGraph->original( RRR.original( RR.original(e).front() )) ];
				if( sub & 1 ) {
					if(	sub & 2 ) {
						GAR.colorEdge(e) = "#000000";
					} else {
						GAR.colorEdge(e) = "#FF0000";
					}  		
				} else {
					GAR.colorEdge(e) = "#00FF00";
			  	} 
			}		
		}
...
void OptimalSimultaneousCrossingMinimizer::Master::hintsKnKuratowskiMinusOne(List<ABA_CONSTRAINT*>& prelist) {
	node n;
	edge e;
...
void OptimalSimultaneousCrossingMinimizer::Master::helperHintsKnAllSubKuratowskis(NodeArray<bool>& aktnodes, node posNode, int num, List<ABA_CONSTRAINT*>& prelist) {
	node n; 
	edge e;
	if(posNode) {
...
void OptimalSimultaneousCrossingMinimizer::Master::hintsKnAllSubKuratowskis(List<ABA_CONSTRAINT*>& prelist) {
...
void OptimalSimultaneousCrossingMinimizer::Master::hintsKnNodeOrder(List<ABA_CONSTRAINT*>& prelist) {
	node n;
...
void OptimalSimultaneousCrossingMinimizer::Master::hintsKnEdgeOrder(List<ABA_CONSTRAINT*>& prelist) {
	edge e;
...
void OptimalSimultaneousCrossingMinimizer::Master::hintsKnmKuratowskiMinusOne(List<ABA_CONSTRAINT*>& prelist) {
	node n;
	edge e;
...
void OptimalSimultaneousCrossingMinimizer::Master::hintsKnExpensiveKuratowski(List<ABA_CONSTRAINT*>& prelist) {
	node n,nn;
	edge e;
...
void OptimalSimultaneousCrossingMinimizer::Master::hintsKnmAllSubKuratowskis(List<ABA_CONSTRAINT*>& prelist) {
	lout(LL_FORCE) << "WARNING: KnmAllSubKuratowskis not implemented! -> no such hints generated!!!";
	OGDF_ASSERT(false);
}

void OptimalSimultaneousCrossingMinimizer::Master::hintsKnmNodeOrder(List<ABA_CONSTRAINT*>& prelist) {
	node n;
	edge e;
...
void OptimalSimultaneousCrossingMinimizer::Master::hintsKnmEdgeOrder(List<ABA_CONSTRAINT*>& prelist) {
	node n;
	edge e;
...
int OptimalSimultaneousCrossingMinimizer::Master::enumerationStrategy(ABA_SUB* s1, ABA_SUB* s2) {
